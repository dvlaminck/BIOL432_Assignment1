species <- sample(c("felis catus","canis lupus familiaris","giraffa camelopardalis",
...
limb_width <- ifelse(species == 'felis cactus', sample(c(1:3), replace = TRUE), #If species is cat, sample from 1-3 for width
                      ifelse(species == 'canis lupus familiaris', sample(c(1:6), replace=TRUE), #If species is dog, sample from 1-6 for width
...
limb_length <- ifelse(species == 'felis catus', sample(c(15:30), replace = TRUE),
                      ifelse(species == 'canis lupus familiaris', sample(c(15:110), replace=TRUE),
